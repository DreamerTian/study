...
func (blc *Blockchain) MineNewBlock(from []string, to []string, amount []string) {
	...
	for index,address := range from{
		...
		tx := NewSimpleTransaction(address, to[index], value,blc,txs)
...
//如果一个地址对应的TXOutput未花费，那么这个Transaction 就应该添加到数组中返回
func (blc *Blockchain) UnUTXOs(address string , txs []*Transaction) []*UTXO {

	//用于存储未花费的
	var unUTXOs[]*UTXO

	spentTxOutputs := make(map[string][]int) //{hash:[0]}
	...
	for {
		blocks := blockIterator.Next()

		fmt.Println(blocks)
		fmt.Println()

		for i := len(blocks.Txs) -1 ; i >= 0 ;i-- {
			// txHash
			tx := blocks.Txs[i]
			//Vins
			if tx.IsCoinbaseTransaction() == false{
		...
		hashInt.SetBytes(blocks.PrevBlockHash)
...
func (blc *Blockchain) GetBalance(address string) int64{
	utxos := blc.UnUTXOs(address,[]*Transaction{})
